...
status
- TODO: convert to standard buffer type
...
static buffer *_huffman_build_compressed_buffer(buffer *uncompressed, huffman_tree *tree);
static huffman_tree *_build_huffman_tree(buffer *uncompressed);
...
// buffer *huffman_buffer_init(size_t length)
// {
//     buffer *buffer = (huffman_buffer *)malloc(sizeof(huffman_buffer));
//     if (buffer == NULL)
//         return NULL;
...
static buffer *_huffman_build_compressed_buffer(buffer *uncompressed, huffman_tree *tree)
{
  ...
  compressed = buffer_init(uncompressed->length);
  compressed->length = 0;
...
buffer *huffman_compress(buffer *uncompressed)
{
  huffman_tree *tree = _build_huffman_tree(uncompressed);
  buffer *compressed = _huffman_build_compressed_buffer(uncompressed, tree);

  return compressed;
}

buffer *huffman_decompress(buffer *compressed, huffman_type tree_type)
{
  buffer *uncompressed;
  huffman_tree *ll_tree = NULL;

  switch (tree_type)
  {
  case BLOCK_TYPE_FIXED_HUFFMAN:
    ll_tree = _build_fixed_tree();
  case BLOCK_TYPE_DYNAMIC_HUFFMAN:
    ll_tree = _build_dynamic_tree(compressed);
  default:
    return NULL;
  }

  uncompressed = _huffman_build_decompressed_buffer(compressed, ll_tree);

  return uncompressed; // FIXME: need to complete
}

static huffman_tree *_build_fixed_tree()
{
  // TODO: This is symbol tree; also need to build distance tree
  huffman_tree *tree;
  ...
  tree->size = 288;
  tree->nodes = (node **)malloc(sizeof(node *) * tree->size);
  if (tree->nodes == NULL)
    goto malloc_error;

  for (size_t n = 0; n < 288; n++)
  {
    tree->nodes[n] = _init_node();
    if (tree->nodes[n] == NULL)
      goto malloc_error;
    ...
    if (n < 144)
      tree->nodes[n]->code_length = 8;
    else if (n < 256)
      tree->nodes[n]->code_length = 9;
    else
      tree->nodes[n]->code_length = 7;
  }

  // TODO: Something special about 256. Also what is termination code?
...
static buffer *_huffman_build_decompressed_buffer(buffer *uncompressed, huffman_tree *tree)
{
}
