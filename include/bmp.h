...
typedef struct bmp_colour_t
{
  ...
  bmp_subpixel green;
  bmp_subpixel red;
  bmp_subpixel unused;              // pad to 32-bit by convention
} bmp_colour;

#define BMP_FILE_HEADER_SIGNATURE {'B', 'M'}
...
#endif
#define bmp_debug_value(pix) fprintf(stderr, #pix " = \n"); \
        bmp_printf(BMP_DEBUG_FORMAT, pix); \
        fprintf(stderr, "\n\n");
#define bmp_debug_command(f) fprintf(stderr, ">> " #f "\n\n"); f;
